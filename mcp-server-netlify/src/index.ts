...
function generateToken(userId: string): string {
  return jwt.sign({ userId }, CONFIG.JWT_SECRET as any, { expiresIn: CONFIG.JWT_EXPIRES_IN } as any);
}
...
const tools = {
  ...
  advanced: {
    ...
    async execute({ arguments: args, token }: any) {
      ...
      const status = await getSubscriptionStatus(userId);
      if (status.plan === 'FREE') throw new Error('🔒 PRO required');
      ...
      switch (args.action) {
        case 'calculate': {
          try {
            ...
            return { result };
          } catch { throw new Error('Invalid expression'); }
        }
        case 'weather':
          return { 
        ...
        case 'sort':
          return { 
            asc: (args.array || []).sort((a: any, b: any) => a - b),
            desc: (args.array || []).sort((a: any, b: any) => b - a)
          };
        case 'filter':
          return {
            even: (args.array || []).filter((n: number) => n % 2 === 0),
            odd: (args.array || []).filter((n: number) => n % 2 !== 0)
          };
        default: throw new Error('Unknown action');
      }
    }
    ...
    async execute({ arguments: args, token }: any) {
      ...
      switch (args.action) {
        ...
        case 'stringify':
          ...
          } catch { throw new Error('Cannot stringify'); }
        case 'validate':
          try {
  ...
  data: {
    ...
    async execute({ arguments: args, token }: any) {
      ...
      switch (args.action) {
        ...
        case 'filter': return { filtered: arr.filter((x: any) => x) };
        case 'map': return { mapped: arr.map((x: number) => x * 2) };
        case 'reduce': return { sum: arr.reduce((a: number, b: number) => a + b, 0) };
  ...
  api: {
    ...
    async execute({ arguments: args, token }: any) {
      ...
      switch (args.action) {
        ...
        case 'validate':
          try { new URL(args.url); return { valid: true }; } catch { throw new Error('Invalid URL'); }
        case 'headers':
  ...
  csv: {
    ...
    async execute({ arguments: args, token }: any) {
      ...
      switch (args.action) {
        ...
        case 'stringify': return { csv: (args.data || []).map((r: any) => Object.values(r).join(',')).join('\n') };
  ...
  search: {
    ...
    async execute({ arguments: args, token }: any) {
      ...
      switch (args.action) {
        ...
        case 'filter':
          return { results: (args.array || []).filter((x: any) => String(x).includes(args.term)) };
        case 'query':
          return { matches: [] };
        case 'regex':
  ...
  database: {
    ...
    async execute({ arguments: args, token }: any) {
      ...
      const status = await getSubscriptionStatus(userId);
      if (status.plan !== 'ENTERPRISE') throw new Error('🔒 ENTERPRISE required');
  ...
  admin: {
    ...
    async execute({ arguments: args, token }: any) {
      ...
      switch (args.action) {
        case 'list_users': return { users: Array.from(users.values()).map((u: any) => ({ email: u.email, plan: u.subscriptionPlan })) };
...
async function executeToolRequest(event: any) {
  ...
  const result = await tools[toolCategory as keyof typeof tools].execute({ arguments: args, token });
  return {
    statusCode: 200,
    headers: { 'Content-Type': 'application/json' },
...
export async function handler(event: any, context: any) {
  const path = String(event.rawPath || event.path || '').toLowerCase();
  const method = String(event.httpMethod || 'GET').toUpperCase();

  if (method === 'GET' && (path === '/health' || path === '/.netlify/functions/index/health')) {
    return health(event, context);
  }

  if (method === 'GET' && (path === '/discovery' || path === '/api/discovery')) {
    return discovery(event, context);
  }

  if (method === 'POST' && (path === '/api/auth/create' || path === '/api/subscribe')) {
    return createAccount(event, context);
  }

  if (method === 'POST' && path === '/api/auth/refresh') {
    return refreshToken(event, context);
  }

  if (method === 'POST' && (path === '/api/mcp' || path.startsWith('/api/mcp/'))) {
    return executeToolRequest(event);
  }
...
// Discovery endpoint
export async function discovery(event: any, context: any) {
  return {
    statusCode: 200,
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      ...
      tools: Object.entries(tools).map(([key, tool]: any) => ({
        name: key,
...
// Create account
export async function createAccount(event: any, context: any) {
  try {
    const body = JSON.parse(event.body || '{}');
    ...
    if (!email || !subscriptionPlan) {
      return {
        statusCode: 400,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Missing fields: email and subscriptionPlan are required' })
      };
    }

    const plan = CONFIG.PLANS[subscriptionPlan as keyof typeof CONFIG.PLANS];
    if (!plan) {
      return {
        statusCode: 400,
        headers: { 'Content-Type': 'application/json' },
    ...
    const userId = uuidv4();
    const subscriptionId = paymentId || `sub_${uuidv4()}`;
    ...
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ success: true, token, userId, plan: subscriptionPlan, subscriptionId })
    };
  } catch (error) {
    return {
      statusCode: 500,
...
// Refresh token
export async function refreshToken(event: any, context: any) {
  try {
    const body = JSON.parse(event.body || '{}');
    const { token } = body;
...
// Health check
export async function health(event: any, context: any) {
  return {
    statusCode: 200,
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
